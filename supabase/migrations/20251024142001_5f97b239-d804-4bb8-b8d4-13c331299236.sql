...
DROP POLICY IF EXISTS "Venue owners can insert their events" ON public.events;
DROP POLICY IF EXISTS "Venue owners can update their events" ON public.events;
DROP POLICY IF EXISTS "Venue owners can delete their events" ON public.events;
...
DROP POLICY IF EXISTS "Users can update their own profile (except business_type)" ON public.profiles;
...
WITH CHECK (
  ...
  (business_type IS NULL OR business_type = (SELECT business_type FROM public.profiles WHERE user_id = auth.uid()))
);
